...
class Brands extends Admin_Controller 
{
	public function __construct()
	{
		...
		$this->data['page_title'] = 'แบรนด์สินค้า';
	...
	*/
	public function fetchBrandData()
	{
		...
		foreach ($data as $key => $value) {
			...
			if(in_array('viewBrand', $this->permission)) {
				$buttons .= '<button type="button" class="btn btn-default" onclick="editBrand('.$value['id'].')" data-toggle="modal" data-target="#editBrandModal"><img src="'.base_url(['assets/icon/photo2.png']).'" width="15"></button>';	
			}
			
			if(in_array('deleteBrand', $this->permission)) {
				$buttons .= ' <button type="button" class="btn btn-default" onclick="removeBrand('.$value['id'].')" data-toggle="modal" data-target="#removeBrandModal"><img src="'.base_url(['assets/icon/delete.png']).'" width="15"></button>
				';
			}				
