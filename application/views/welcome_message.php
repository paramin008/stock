						...
						<?php foreach($items as $row) : ?>
							<tr>
								...
								<td>
									<div class="row">
										<div class="col">
											<a href="<?= base_url("reorder/index/{$row->id}") ?>" class="btn btn-warning btn-block btn-sm">
												แก้ไข
											</a>
										</div>
										<div class="col">
											<a onclick="return confirm('คุณต้องการจะลบข้อมูลนี้จริงหรือ?')" href="<?= base_url("reorder/delete/{$row->id}") ?>" class="btn btn-danger btn-block btn-sm">
												ลบทิ้ง
											</a>
											<a target="__blank" href="<?php echo base_url() . 'reorder/printDiv/'.$row->id?>" class="btn btn-default" >Print</a>
										</div>
									</div>
								</td>
							</tr>
